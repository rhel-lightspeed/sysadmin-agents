...
# Create the upgrade agent with MCP tools
# Uses YAML for config, programmatic creation for MCP (ADK workaround)
upgrade_agent = create_agent_with_mcp(CONFIG_PATH)
logger.info(f"Upgrade agent created: {upgrade_agent.name}")

# Alias for ADK web discovery
root_agent = upgrade_agent
